...
if [ ! -s "$NESTORA_ENV" ]; then
  ...
  NESTORA_MALWARE_SCAN_TOKEN="$(openssl rand -hex 32)"
  NESTORA_DELIVERY_WEBHOOK_TOKEN="$(openssl rand -hex 32)"
  NESTORA_JOB_SECRET_VALUE="$(openssl rand -hex 48)"
  cat > "$NESTORA_ENV" <<EOF
NODE_ENV=production
PORT=3003
...
NESTORA_STORAGE_BUCKET=nestora-media-prod
AWS_REGION=us-east-2
AWS_ACCESS_KEY_ID=
AWS_SECRET_ACCESS_KEY=
NESTORA_MALWARE_SCAN_URL=https://scanner.doctarx.com/scan
NESTORA_MALWARE_SCAN_TOKEN=${NESTORA_MALWARE_SCAN_TOKEN}
NESTORA_DELIVERY_WEBHOOK_URL=https://doctarx.com/api/internal/delivery
NESTORA_DELIVERY_WEBHOOK_TOKEN=${NESTORA_DELIVERY_WEBHOOK_TOKEN}
NESTORA_JOB_SECRET=${NESTORA_JOB_SECRET_VALUE}
