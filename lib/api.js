    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (!path.includes('/login')) {
          ...
          let loginUrl = '/patient/login?session=expired';
          if (path.includes('/admin') || path.includes('/secure')) {
            loginUrl = '/secure/admin?session=expired';
          } else if (path.includes('/ng/provider')) {
            loginUrl = '/ng/provider/login?session=expired';
          } else if (path.includes('/provider')) {
            loginUrl = '/provider/login?session=expired';
          }
