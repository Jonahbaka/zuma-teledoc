...
const getDefaultVideoConstraints = () => ({
  width: { ideal: 1280 },
  height: { ideal: 720 },
  facingMode: 'user'
});
...
export function useTelehealthSession({ appointmentId } = {}) {
  ...
  const ensureLocalStream = useCallback(async ({ video = true, audio = true } = {}) => {
    const wantsVideo = video !== false;
    const wantsAudio = audio !== false;
    const existingStream = localStreamRef.current;

    if (
      existingStream?.getTracks().some((track) => track.readyState === 'live') &&
      (!wantsVideo || hasLiveTrack(existingStream, 'video')) &&
      (!wantsAudio || hasLiveTrack(existingStream, 'audio'))
    ) {
    ...
    if (existingStream?.getTracks().some((track) => track.readyState === 'live')) {
      const needsVideo = wantsVideo && !hasLiveTrack(existingStream, 'video');
      const needsAudio = wantsAudio && !hasLiveTrack(existingStream, 'audio');

      if (needsVideo || needsAudio) {
        try {
          const addedStream = await navigator.mediaDevices.getUserMedia({
            video: needsVideo ? getDefaultVideoConstraints() : false,
            audio: needsAudio
          });
          const combinedStream = new MediaStream([
            ...existingStream.getTracks().filter((track) => track.readyState === 'live'),
            ...addedStream.getTracks()
          ]);
          ...
          if (peerRef.current) {
            syncLocalTracks(peerRef.current, combinedStream);
          }

          return combinedStream;
        } catch (trackError) {
          const finalMessage = getMediaErrorMessage(trackError);
    ...
    try {
      stream = await navigator.mediaDevices.getUserMedia({
        video: wantsVideo ? getDefaultVideoConstraints() : false,
        audio: wantsAudio
      });
    } catch (primaryError) {
      if (!wantsVideo || !wantsAudio) {
  ...
  const connectSocket = useCallback(async () => {
    ...
    socket.on('telehealth:signal', async ({ fromSocketId, participant, signalType, signal }) => {
      try {
        ...
        await ensureLocalStream(joinStateRef.current.mediaOptions || {});
  ...
  const joinRoom = useCallback(async (displayName, mediaOptions = {}) => {
    if (!appointmentId) {
      throw new Error('Appointment ID is required to start a video call');
    }

    await ensureLocalStream(mediaOptions);
  ...
  const joinCall = useCallback(async ({ displayName, mediaOptions } = {}) => {
    setCallError(null);
    setStatus('joining');
    joinStateRef.current.displayName = displayName || '';
    joinStateRef.current.mediaOptions = mediaOptions || {};
    await joinRoom(displayName || '', mediaOptions || {});
  }, [joinRoom, setStatus]);
  ...
  rejoinRef.current = async () => {
    ...
    closePeerConnection({ clearRemote: true });
    await joinRoom(joinStateRef.current.displayName, joinStateRef.current.mediaOptions || {});
  };
